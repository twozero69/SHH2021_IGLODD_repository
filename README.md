...
## 팀원소개 및 역할분담
이영: 팀장, 라이다를 이용한 손가락 인식프로그램 개발

전상현: HW 설계

이준기: step모터 구동 펌웨어 개발

김동욱: 키오스크 GUI프로그램 개발, 적외선 센서 펌웨어 개발

## 시스템 구성도
![시스템구성도](https://user-images.githubusercontent.com/60468060/103454535-81676680-4d28-11eb-9f01-d4409b5699af.png)

## 구현하지 못한 기능
stm32CubeMX가 제공하는 USB_DEVICE(HID)기능을 통해 마우스를 제어하려고 하였으나 한번에 -127~127픽셀만큼만 이동이 가능하여 1920X1080의 모니터에서는 사용하기 부적합하다고 판단하여 라즈베리파이에서 파이썬 라이브러리 pyautogui를 사용하여 구현하였습니다.

## 소스코드
### IGLODDer
IGLODD팀의 B-L4S5I-IOT01A보드의 펌웨어로 적외선 센서의 값을 받고 스텝모터를 구동시키는 프로그램이 구현되어 있습니다. 스텝 수를 세어 범위에서 벗어나지 않도록 구현되어 있는 것이 특징입니다.

### twozeroLidar
2D라이다의 데이터로부터 디스플레이의 위에 위치한 손가락을 탐지하는 프로그램입니다.

### twozeroClick
탐지된 손가락 위치에 대하여 마우스 이동, 클릭을 구현한 프로그램입니다.

### twozeroClick2
클릭없이 탐지된 손가락 위치에 대하여 마우스를 이동시키는 테스트용 프로그램입니다.

### GUI.py
메뉴선택, 결제가 가능한 간단한 GUI프로그램입니다.. 결제버튼을 클릭하면 주문내역이 AWS IoT로 업로드 됩니다.
...
## 기존제품과의 비교
자료조사 결과 동일한 주제의 제품이 미국 GTT사(http://gtt.co.kr/en/product/untacttouch.php )에 존재합니다.
